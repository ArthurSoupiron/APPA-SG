...
export const actionPlanSubAction = actionPlanSchema.table(
  "sub_action",
  {
  ...
  (t) => [
    index("sub_action_action_id_idx").on(t.actionId),
    index("sub_action_status_idx").on(t.status),
    index("sub_action_sort_order_idx").on(t.sortOrder),
    check("sub_action_progress_check", sql`${t.progress} >= 0 AND ${t.progress} <= 100`),
    check(
      "sub_action_status_check",
      sql`${t.status} IN (${sql.raw(ACTION_PLAN_STATUS_SQL)})`,
    ),
    check(
      "sub_action_campus_check",
