...
import {
  GW_DIRECTORY_SYNC_KIND,
  startGwDirectorySyncJob,
} from "../../lib/gw-directory-sync";
...
export function registerGwOpsRoutes(
  ...
  adminBanners.get("/", async (c) => {
    ...
    const rows = await db
      .select()
      .from(systemBanner)
      .orderBy(desc(systemBanner.createdAt));
    return c.json({
      banners: rows.map((b) => ({
        id: b.id,
        title: b.title,
  ...
  adminBanners.patch("/:id", async (c) => {
    ...
    if (typeof patch.startsAt === "string")
      updates.startsAt = new Date(patch.startsAt);
    if (patch.endsAt === null) updates.endsAt = null;
    else if (typeof patch.endsAt === "string")
      updates.endsAt = new Date(patch.endsAt);
