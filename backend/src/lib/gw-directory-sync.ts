...
function scopeLikelyIncludesDirectory(
...
async function getDirectoryAuth(
  ...
  | { ok: true; auth: InstanceType<typeof google.auth.OAuth2> }
...
> {
  ...
  if (!clientId || !clientSecret) {
    return {
      ok: false,
      message:
        "GOOGLE_CLIENT_ID / GOOGLE_CLIENT_SECRET manquants côté backend.",
    };
  }
  ...
  const [googleAccount] = await db
    ...
    .from(account)
    .where(
      and(eq(account.userId, adminUserId), eq(account.providerId, "google")),
    )
    .limit(1);
  ...
  if (
    ...
    accessTokenStillValid(googleAccount.accessTokenExpiresAt)
  ) {
    oauth2.setCredentials({
 ...
 */
export function startGwDirectorySyncJob(
  jobId: string,
  adminUserId: string,
): void {
  void (async () => {
    try {
      ...
      const knownGroupIds = new Set(
        groups.map((g) => g.id).filter((x): x is string => Boolean(x)),
      );
      ...
      for (const g of groups) {
        ...
        await db
          ...
          .where(eq(workspaceGroupMember.containerGroupId, gid));
