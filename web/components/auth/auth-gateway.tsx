...
type AuthGatewayInnerProps = {
  ...
  compact?: boolean;
  /** `null` = chargement config (formulaires actifs par défaut). */
  emailPasswordEnabled: boolean | null;
};

const EMAIL_PASSWORD_DISABLED_MESSAGE =
  "La connexion par email et mot de passe est désactivée. Utilisez Google.";
...
function AuthGatewayInner({
  ...
  const emailPasswordDisabled = emailPasswordEnabled === false;

  return (
    <Card
      className={
        compact
          ? "flex w-full min-w-0 flex-col gap-0 border-0 bg-transparent py-0 shadow-none ring-0"
    ...
    >
      ...
      <CardContent
        className={compact ? "flex flex-col px-0 pt-0" : "space-y-6"}
      >
        <Tabs onValueChange={(v) => setTab(v as AuthTab)} value={tab}>
          ...
          <TabsContent className={compact ? "mt-2 space-y-2" : "mt-4 space-y-4"} value="signin">
            <SignInForm
              ...
              disabledMessage={emailPasswordDisabled ? EMAIL_PASSWORD_DISABLED_MESSAGE : null}
...
export function AuthGatewayLanding({
  ...
  const [emailPasswordEnabled, setEmailPasswordEnabled] = useState<boolean | null>(null);
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (!res.ok) return;
        const data = (await res.json()) as { emailPasswordEnabled?: boolean };
        if (!cancelled && typeof data.emailPasswordEnabled === "boolean") {
          setEmailPasswordEnabled(data.emailPasswordEnabled);
        }
      } catch {
        // Garde le défaut permissif (formulaires actifs).
      }
    })();
    return () => {
      cancelled = true;
  ...
  if (embedded) {
    return <div className="relative flex w-full flex-col">{inner}</div>;
  }
    ...
    <form className={compact ? "space-y-2.5" : "space-y-4"} onSubmit={onSubmit}>
      ...
      {disabledMessage ? (
        <PretextBlock
          className={
            compact
              ? "text-muted-foreground text-xs whitespace-normal wrap-break-word"
              : "text-muted-foreground text-sm whitespace-normal wrap-break-word"
          }
          metric={compact ? PRETEXT.xs : PRETEXT.sm}
          role="status"
