...
import { publishWebflowItem, pullWebflowBlogItems } from "../../lib/marketing/webflow-client";
import type { AppVariables } from "../../types/app";

export function registerMarketingWebflowRoutes(app: HonoType<{ Variables: AppVariables }>) {
  ...
  r.post("/webflow/blog/sync", async (c) => {
    ...
    const env = getMarketingEnv();
    if (!env.webflowSiteId) return c.json({ error: "WEBFLOW_SITE_ID manquant" }, 400);

    try {
      const result = await pullWebflowBlogItems(env.webflowSiteId);
      return c.json(result);
    } catch (e) {
      return c.json({ error: e instanceof Error ? e.message : "sync_failed" }, 400);
    }
  });
  ...
  r.patch("/webflow/blog/:id", async (c) => {
    ...
    await db
      ...
      .set({
        ...
        categories: typeof body.categories === "string" ? body.categories : existing.categories,
  ...
  r.post("/webflow/blog/:id/publish", async (c) => {
    ...
    try {
      ...
      return c.json({ error: e instanceof Error ? e.message : "publish_failed" }, 400);
    }
  });
