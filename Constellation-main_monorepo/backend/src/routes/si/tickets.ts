...
import { uploadFileToSiTicketFolder, ensureSiTicketFolderPath } from "../../lib/si-drive";
import {
  canAgentTransition,
...
import { ensureCreatorWatcher, notifyTicketWatchers } from "../../lib/si/ticket-notifications";
import {
...
export function registerSiTicketRoutes(app: HonoType<{ Variables: AppVariables }>) {
...
async function listTickets(c: SiCtx, opts: { mine?: boolean; all?: boolean }) {
  ...
  if (q) {
    conditions.push(
      or(ilike(ticket.title, `%${q}%`), ilike(ticket.reference, `%${q}%`), ilike(ticket.description, `%${q}%`)),
    );
  }
...
async function createTicket(c: SiCtx) {
  ...
  const driveRes = await ensureSiTicketFolderPath(user.id, reference, title, createdAt);
  if (!driveRes.ok) {
    return c.json({ error: driveRes.code ?? "drive_error", message: driveRes.message }, 502);
  }
  ...
  for (const file of files) {
    const up = await uploadFileToSiTicketFolder(user.id, driveRes.folder.folderId, file);
    if (up.ok) {
  ...
  queueMicrotask(() => {
    void exportTicketEvent(user.id, ticketId, "created", user.id, { reference, title });
  });
...
async function patchTicket(c: SiCtx) {
  ...
  if (body?.title !== undefined || body?.description !== undefined) {
    ...
    if (body.title !== undefined) {
      ...
      if (!t || t.length > MAX_TITLE) return c.json({ error: "invalid_title" }, 400);
      updates.title = t;
    }
    if (body.description !== undefined) {
      const d = body.description.trim();
      if (!d || d.length > MAX_DESCRIPTION) return c.json({ error: "invalid_description" }, 400);
      updates.description = d;
    }
  }

  if (body?.status !== undefined) {
    if (!agent) return c.json({ error: "forbidden", need: "si.ticket.manage" }, 403);
    const to = body.status as SiTicketStatus;
    if (!SI_TICKET_STATUSES.includes(to)) return c.json({ error: "invalid_status" }, 400);
    ...
    queueMicrotask(() => {
      void exportTicketEvent(user.id, id, "status_change", user.id, { from, to });
    });
  }
  ...
  if (body?.assigneeUserId !== undefined) {
    ...
    await notifyTicketWatchers({
      ticketId: id,
      kind: "assigned",
      payload: { assigneeUserId: body.assigneeUserId, reference: row.reference },
      excludeUserId: user.id,
    });
    queueMicrotask(() => {
...
async function addComment(c: SiCtx) {
  ...
  if (!text || text.length > MAX_COMMENT) return c.json({ error: "invalid_comment" }, 400);
  ...
  queueMicrotask(() => {
    void exportTicketEvent(user.id, id, "comment", user.id, { body: text.slice(0, 500) });
  });
...
async function addAttachment(c: SiCtx) {
  ...
  for (const file of parsed) {
    const up = await uploadFileToSiTicketFolder(user.id, row.driveFolderId, file);
    if (up.ok) {
      ...
      uploadWarnings.push(`${file.name}: ${up.message}`);
      console.error("[si.ticket] upload attachment failed", file.name, up.message);
    }
  }

  if (uploaded.length === 0) {
    return c.json(
      { error: "upload_failed", message: uploadWarnings.join(" ; ") || "Échec Drive." },
      502,
    );
  }

  queueMicrotask(() => {
    void exportTicketEvent(user.id, id, "attachment", user.id, { count: uploaded.length });
  });
...
async function toggleWatcher(c: SiCtx) {
  ...
  const body = (await c.req.json().catch(() => null)) as { watch?: boolean } | null;
  const watch = body?.watch !== false;

  if (watch) {
    await db.insert(ticketWatcher).values({ ticketId: id, userId: user.id }).onConflictDoNothing();
  } else {
    await db
      .delete(ticketWatcher)
      .where(and(eq(ticketWatcher.ticketId, id), eq(ticketWatcher.userId, user.id)));
  }
...
async function runRecoveryImportJob(jobId: string, userId: string) {
  ...
  try {
    const result = await runSiTicketSheetRecovery(userId, async (pct, label) => {
      await db
        .update(asyncJob)
...
async function runRecoveryExportJob(jobId: string, userId: string) {
  ...
  try {
    ...
    const sheetsApiDisabled = result.errors.some(
      (e) => e.includes("sheets.googleapis.com") && /disabled|not been used/i.test(e),
    );
    const jobFailed =
