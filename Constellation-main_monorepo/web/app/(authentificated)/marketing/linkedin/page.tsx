...
export default function MarketingLinkedInPage() {
  ...
  const sync = async (force = false) => {
    ...
    try {
      ...
      await load();
      toast.success(force ? "Synchronisation forcée terminée" : "Données à jour");
    } catch (e) {
      ...
      <Card>
        <CardHeader className="flex flex-row flex-wrap items-center justify-between gap-2">
          ...
          <div className="flex flex-wrap gap-2">
            <Button type="button" variant="outline" disabled={loading} onClick={() => void sync(false)}>
              Actualiser
            </Button>
            <Button type="button" disabled={loading} onClick={() => void sync(true)}>
              Forcer la sync
            </Button>
          </div>
        </CardHeader>
        <CardContent className="space-y-3 text-sm whitespace-normal break-words">
          {!data?.configured && (
            <p className="text-muted-foreground">
              Configurez LINKEDIN_ORGANIZATION_ID et LINKEDIN_ACCESS_TOKEN (voir docs/marketing-integrations.md).
            </p>
          )}
          {data?.latest && (
            <p className="text-muted-foreground">
              Dernière sync : {new Date(data.latest.fetchedAt).toLocaleString("fr-FR")}
            </p>
          )}
