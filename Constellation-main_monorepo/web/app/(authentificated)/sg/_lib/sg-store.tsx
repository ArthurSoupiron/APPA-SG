...
const STORE_KEY = "jeece-sg-store-v1";

function hydrate(): SgData {
  const seed = buildSeed();
  if (typeof window === "undefined") return seed;
  try {
    const raw = window.localStorage.getItem(STORE_KEY);
    if (!raw) return seed;
    const saved = JSON.parse(raw) as Partial<SgData>;
    return {
      members: Array.isArray(saved.members) ? saved.members : seed.members,
      docs: Array.isArray(saved.docs) ? saved.docs : seed.docs,
      deadlines: Array.isArray(saved.deadlines) ? saved.deadlines : seed.deadlines,
      conformite: Array.isArray(saved.conformite) ? saved.conformite : seed.conformite,
      docTypes: Array.isArray(saved.docTypes) ? saved.docTypes : seed.docTypes,
      gedCats: Array.isArray(saved.gedCats) ? saved.gedCats : seed.gedCats,
      activity: Array.isArray(saved.activity) ? saved.activity : seed.activity,
    };
  } catch {
    return seed;
  }
...
type SgContextValue = {
  data: SgData;
  /** applique une transformation immuable de l'état puis persiste */
