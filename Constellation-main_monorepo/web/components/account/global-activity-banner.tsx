...
export function GlobalActivityBanner() {
  ...
  return (
    ...
    >
      {banners.map((b) => (
        <Alert key={b.id} className={cn("rounded-xl", severityClass(b.severity))}>
          <HugeiconsIcon icon={AlertCircleIcon} strokeWidth={2} className="size-4" />
          <AlertTitle>{b.title}</AlertTitle>
          <AlertDescription className="whitespace-pre-wrap">{b.body}</AlertDescription>
        </Alert>
      ))}
      {jobs.map((j) => (
        <Alert key={j.id} className="rounded-xl border-primary/25 bg-muted/30">
          ...
          <AlertDescription className="space-y-2">
            ...
            {j.error ? <span className="text-destructive text-xs">{j.error}</span> : null}
          </AlertDescription>
        </Alert>
      ))}
    </div>
